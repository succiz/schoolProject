...
public class JokeDBAdapter {
	...
	public JokeDBAdapter(Context context) {
		// TODO
		m_dbHelper = new JokeDBHelper(context,JokeDBAdapter.DATABASE_NAME,null,
				JokeDBAdapter.DATABASE_VERSION); 
	}
	...
	public long insertJoke(Joke joke) {
		...
		long ID = m_db.insert(JokeDBAdapter.DATABASE_TABLE_JOKE,null,myContentValues);
		return ID;
	}
	...
	public Cursor getAllJokes(String ratingFilter) {
		...
		return m_db.query (JokeDBAdapter.DATABASE_TABLE_JOKE, 
				new String[]{JokeDBAdapter.JOKE_KEY_ID, 
				JokeDBAdapter.JOKE_KEY_TEXT, 
				JokeDBAdapter.JOKE_KEY_RATING,
				JokeDBAdapter.JOKE_KEY_AUTHOR},
	...
	public static Joke getJokeFromCursor(Cursor cursor) {
		// TODO
		Joke myJoke = null;
		if(cursor.isAfterLast()==true||cursor == null)
			return myJoke;
		else{
			myJoke = new Joke();
		...
		myJoke.setAuthor(author);
		myJoke.setID(id);
		myJoke.setJoke(text);
		myJoke.setRating(rating);
	...
	public boolean updateJoke(Joke joke) {
		...
		int count;
		count = m_db.update(JokeDBAdapter.DATABASE_TABLE_JOKE , myContentValues, string, null);
		if(count > 0)
			return true;
		else 
			return false;
