/*Name Shuyi Zhao
  ...
  Acknowledge that the source code is from Malik, chapter 5
*/
//New variables: private node currentNode
//New functions: copylist, getItemForward, and getItemReverse
...
public:
	...
	//Fountion to retrieve the next element from the linked list in forward order.
	//to print the list in forward order with formatting.
	
    Type getItemReverse(int index);
	//Founction to retrieve the next element from the linked list in backward order.
	//to print the list in forward order with formatting.
...
private:
    void copyList(const doublyLinkedList<Type>& otherList); 
      ...
      //    to this list.
    nodeType<Type> *currentNode;//pointer to the current node
};
...
void doublyLinkedList<Type>::insert(const Type& insertItem)
{
	...
	if(search(insertItem))
	 {  cout<<insertItem<<" has already been here."<<endl;
	    return;
	 }
	else
    {
...
void doublyLinkedList<Type>::copyList(const doublyLinkedList<Type>& otherList) 
{
    ...
    else
    {
        ...
        while (current != NULL)
        {
            ...
            newNode->back = last;       //set the back of newNode to last
...
Type doublyLinkedList<Type>::getItemReverse(int index)
{
 if(index == length())
   {
      currentNode=last;
      currentNode->info=last->info;
      currentNode->back=last->back;
