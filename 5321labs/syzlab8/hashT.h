/*Name Shuyi Zhao
  ...
  Acknowledge that the source code is from Malik, chapter 9
*/
#ifndef H_Htable
#define H_Htable
...
public:
	...
	int getLength();
	  //function to get the length of hash table
	  
    void search(int& hashIndex, const elemType& rec, bool& found, 
			int& pCount, linkedQueueType<elemType>& probeSequence) const;
...
{
    cout<<setw(15)<<"indexStatusList"<<setw(15)<<"HashTable"<<endl;
    for (int i = 0; i < HTSize; i++)
       {
            if (indexStatusList[i] == 1)
                cout << setw(4) << i << setw(5) << indexStatusList[i]<< setw(15) 
                     << i << setw(5) << HTable[i] << endl;
            if (indexStatusList[i] == 0)
                cout << setw(4) << i << setw(5) << indexStatusList[i] <<setw(15) 
                     << i << endl;
            if (indexStatusList[i] == -1)
                cout << setw(4) << i << setw(5) << indexStatusList[i]
                     << setw(15) <<  i << setw(5) << HTable[i] << endl;
       }
}
			...
			int& pCount, linkedQueueType<elemType>& probeSequence) const
{
	...
	{
		pCount++;
		probeSequence.addQueue(hashIndex);
		hashIndex = (hashIndex + inc ) % HTSize;
		inc = inc + 2;
	}
