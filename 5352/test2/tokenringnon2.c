...
{
	int rank, nprocs;
	int i, token1, token2, tag1 = 11, tag2 = 22, rounds, flag1 = 0, flag2 = 0, flag_p1 = 0, flag_p2 = 0;
	int left, right;
    ...
    if (rank == 0)
	{
		MPI_Isend(&token1, 1, MPI_INT, right, tag1, MPI_COMM_WORLD, &reqs1[1]);
		MPI_Isend(&token2, 1, MPI_INT, left, tag2, MPI_COMM_WORLD, &reqs2[1]);
	}
	...
	for (i= 0; i < rounds; i++)
	{
		...
		MPI_Irecv(&token1, 1, MPI_INT, left, tag1, MPI_COMM_WORLD, &reqs1[0]);
		
		//MPI_Test(&reqs1[0], &flag1, status1);
